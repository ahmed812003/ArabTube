...
namespace ArabTube.Services.ControllersServices.AuthServices.ImplementationClasses
{
    public class AuthService : IAuthService
    {
        ...
        // done
        public async Task<ProcessResult> EmailConfirmationAsync(string userId , string validCode , string userCode)
        {
            if (string.IsNullOrEmpty(userId))
            {
                return new ProcessResult { Message = "Invalid User Id" };
            }

            if (string.IsNullOrEmpty(validCode) || validCode != userCode)
            {
                return new ProcessResult { Message = "Invalid Code" };
            }
        ...
        //done
        public async Task<ProcessResult> ResetPasswordAsync(string userId, string validCode , string userCode, string newPassword)
        {
            if (string.IsNullOrEmpty(userId))
            {
                return new ProcessResult { Message = "Invalid User Id" };
